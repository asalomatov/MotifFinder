...
//		Element(population,i,0) = RandomUniform(min[0],max[0]);
//		Element(population,i,1) = RandomUniform(min[0],Element(population,i,0));

//		for (int j=2; j < nDim; j++)
//		{	Element(population,i,j) = RandomUniform(min[j],Element(population,i,j-1)*(lim[j]-lim[j-1])/(lim[j-1]-lim[j-2]));

//		std::cout<<"initial population setup: "<<std::endl;
//		std::cout << Element(population,i,j) << std::endl;
//		}
...
//	std::cin >> hangon;
	...
	for (i=0; i < nDim; i++)
		bestSolution[i] = 0.5*(min[i]+max[i]); //;nDim-i
...
//std::cout << trialSolution[n] << std::endl;
//std::cin >> hangon;
